...
class Grenade extends Movement {
    constructor(angle, power) {
        ...
        this.start = (userCoordinates, startTurn) => {
            this.startCoordinates(userCoordinates);
            this.moveTurns = {
                start: startTurn,
                first: startTurn + 3,
        ...
        this.killPlayersInBlastRadius = (user, badgers) => {
    ...
    moveGrenade(turn) {
        const { first, second, third, firstBlast, secondBlast, thirdBlast, dead, end, } = this.moveTurns;
        switch (turn) {
            case first:
                this.fullMovement();
                break;
